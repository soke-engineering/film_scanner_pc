...
static FT_STATUS sendJtagCommand(FT_HANDLE ftHandle, unsigned char *sequence, const size_t length)
{
...
static FT_STATUS setupJtagLoopback(FT_HANDLE ftHandle, DWORD bytesToWrite)
{
    FT_STATUS     ftStatus = FT_OK;
...
int main(int argc, char *argv[])
{
    ...
    {
        printf(
            "Failure.  FT_Read only read %d (of %d) bytes.\n", (int)bytesRead, (int)bytesReceived);
        goto exit;
    }
