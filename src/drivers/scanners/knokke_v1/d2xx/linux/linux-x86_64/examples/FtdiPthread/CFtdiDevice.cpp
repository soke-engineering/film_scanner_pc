...
CFtdiDevice::~CFtdiDevice() { StopThreads(); }
...
void CFtdiDevice::OpenDevice(int deviceNumber)
{
    ...
    if (status == FT_OK)
    {
        ...
        std::cout << "Device " << m_serial << " FT_SetEventNotification result " << ftStatus
                  << std::endl;
    }
    else
    {
...
void CFtdiDevice::CloseDevice()
{
    ...
    {
        std::cout << "Failed to close device " << m_serial << " has handle " << m_handle
                  << std::endl;
    }
...
DWORD CFtdiDevice::GetNumBytesAvailable()
{
    ...
    FT_STATUS ftStatus =
        FT_GetStatus(m_handle, &numRxBytesAvailable, &numTxBytesAvailable, &eventStatus);

    if (FT_OK != ftStatus)
    {
        std::cout << "Device " << m_serial << " FT_GetStatus Failed, FT_STATUS: " << ftStatus
                  << std::endl;
    }
    else
    {
...
void CFtdiDevice::ReadThread()
{
    ...
    while (m_handle != nullptr)
    {
        ...
        {
            m_maxRxBytes =
                (numRxBytesAvailable > m_maxRxBytes) ? numRxBytesAvailable : m_maxRxBytes;
            if (logMessage)
            {
                std::cout << "Device " << m_serial << " FT_GetStatus said " << numRxBytesAvailable
        ...
        if (numRxBytesAvailable == 0)
        {
            ...
            if (retValue != 0)
            {
                std::cout << "Device " << m_serial << " WaitForData complete. Result: " << retValue
                          << ", total Tx " << m_totalTxBytes << ", total Rx " << m_totalRxBytes
                          << ", maxBytes " << m_maxRxBytes << std::endl;
                logMessage = true;
            }
            continue;
        }
        ...
        if (FT_OK != ftStatus)
        {
            std::cout << "Device " << m_serial << " FT_Read Failed, FT_STATUS: " << ftStatus
                      << std::endl;
            break;
        }
...
void CFtdiDevice::IsTileThread()
{
    unsigned char txBuffer[] = {
        0xFF, 0x1B, 0x01, 0x02, 0x1E, 0xFF, 0x63, 0x02, 0x01, 0x00, 0x66, 0xFF, 0x63,
        0x02, 0x01, 0x00, 0x66, 0xFF, 0x63, 0x02, 0x01, 0x00, 0x66, 0xFF, 0x63, 0x02,
        0x01, 0x00, 0x66, 0xFF, 0x63, 0x02, 0x01, 0x00, 0x66, 0xFF, 0x63, 0x02, 0x01,
        0x00, 0x66, 0xFF, 0x63, 0x02, 0x01, 0x00, 0x66, 0xFF, 0x63, 0x02, 0x01, 0x00,
        0x66, 0xFF, 0x63, 0x02, 0x01, 0x00, 0x66, 0xFF, 0x63, 0x02, 0x01, 0x00, 0x66,
    };
    ...
    while (m_handle != nullptr)
    {
        ...
        if (FT_OK != ftStatus)
        {
            std::cout << "Device " << m_serial << " FT_Write Failed, FT_STATUS: " << ftStatus
                      << std::endl;
            break;
        }
...
EVENT_HANDLE *CFtdiDevice::NativeCreateFtdiEventHandle()
{
    ...
    {
        LogMessage(
            "NativeCreateFtdiEventHandle call to pthread_cond_init failed with error code: " +
            std::to_string(condRetVal));
    ...
    {
        LogMessage(
            "NativeCreateFtdiEventHandle call to pthread_mutex_init failed with error code: " +
            std::to_string(mutexRetval));
...
int CFtdiDevice::NativeEventTimedWait(EVENT_HANDLE *pEventHandle, int timeoutSecs, int timeoutMs)
{
    ...
    // Convert the specified timeoutSecs and timeoutMs into a timespec structure for use with
    // pthread_cond_timedwait. We do this before we perform the lock, to prevent unnecessary time
    // inside the lock.

    // https://www.ibm.com/docs/en/zos/2.3.0?topic=lf-pthread-cond-timedwait-pthread-cond-timedwait64-wait-condition-variable
    //
    // Below is what the timespec structure is expected to contain for our call to
    // pthread_cond_timedwait. We're not currently using the '64' long version of the methods, we
    // will need to work this out in future.
    //
    // tv_sec
    //      The time to wait for the condition signal. It is expressed in seconds from midnight,
    //      January 1, 1970 UTC. The value specified must be greater than or equal to current
    //      calendar time expressed in seconds since midnight, January 1, 1970 UTC and less than 2,
    //      147, 483, 648 seconds.
    // tv_nsec
    //      The time in nanoseconds to be added to tv_sec to determine when to stop waiting.
    //      The value specified must be greater than or equal to zero(0) and less than 1, 000, 000,
    //      000 (1, 000 million).
    //
    ...
    if (timeRetVal != 0)
    {
        LogMessage("NativeEventTimedWait call to clock_gettime failed with error code: " +
                   std::to_string(timeRetVal));

        // Fail early, as we can't continue without absolute time measurement.
        return false;
    }

    // Add in the required timeout, converting from milliseconds to nanoseconds (multiplying by 1
    // million).
    timeout.tv_sec += timeoutSecs;
    timeout.tv_nsec += (timeoutMs * 1000000);

    // Check that we didn't go over the one thousand million barrier, otherwise wrap around, and add
    // on one second for doing so.
    ...
    do
    {
        ...
        auto getMutexDuration   = std::chrono::duration_cast<std::chrono::milliseconds>(
                                    afterGettingMutex - beforeGettingMutex)
                                    .count();

        if (getMutexDuration > 10)
        {
            std::cout << m_serial << " acquiring mutex took " << getMutexDuration << "ms."
                      << std::endl;
        }
        ...
        if (mutexStatus != 0)
        {
            LogMessage("NativeEventTimedWait call to pthread_mutex_lock failed with error code: " +
                       std::to_string(mutexStatus));

            // Locking failed, can't call pthread_cond_wait and no need to unlock so return failure
            // here.
            return false;
        }

        // Attempt to perform timed wait. This can often report timeout with ETIMEDOUT, which we can
        // ignore as this is a common return code when the tile is idle and not sending data.
        auto beforeThreadWait = std::chrono::high_resolution_clock::now();
        waitRetVal =
            pthread_cond_timedwait(&(pEventHandle->eCondVar), &(pEventHandle->eMutex), &timeout);
        auto afterThreadWait = std::chrono::high_resolution_clock::now();
        auto waitDuration = std::chrono::duration_cast<std::chrono::milliseconds>(afterThreadWait -
                                                                                  beforeThreadWait)
                                .count();
        ...
        if (mutexStatus != 0)
        {
            LogMessage("pthread_mutex_unlock failed with error code: " +
                       std::to_string(mutexStatus));
            return false;
        }
        ...
        if (waitDuration > 200)
        {
            ...
            std::cout << "### " << m_serial << ". pThread wait took " << waitDuration
                      << "ms. RxBytes available: " << bytesAvailable << std::endl;
        }
        ...
        else if (waitRetVal != 0)
        {
            ...
            LogMessage("NativeEventTimedWait call to pthread_cond_wait failed with: " +
                       std::to_string(waitRetVal));
        }
        else
        {
            return waitRetVal;
    ...
    // Successful when pthread_cond_timedwait reports 0 for success or ETIMEDOUT for a typical
    // timeout when tile is idle.
    return waitRetVal;
}
