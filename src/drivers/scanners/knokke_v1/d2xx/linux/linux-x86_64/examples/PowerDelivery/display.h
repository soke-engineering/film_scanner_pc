...
static void DisplayEepromPD(FT_EEPROM_PD *PD_EEProm, FT_DEVICE deviceType)
{
    ...
    printf("\nBM Configuration: 0 for \"Drive Low\", 1 for \"Drive Hi\", 2 for \"Input Mode\", 3 "
           "for \"Don't Care\" \n");
    for (int i = 0; i < 7; i++)
    {
        printf("\tPDO1_GPIO[%d]     : %s \n",
               i,
               PD_EEProm->PDO1_GPIO[i] == GPIO_DONT_CARE
                   ? "DONT_CAR"
                   : (sprintf(str, "%d", PD_EEProm->PDO1_GPIO[i]), str));
        printf("\tPDO2_GPIO[%d]     : %s \n",
               i,
               PD_EEProm->PDO2_GPIO[i] == GPIO_DONT_CARE
                   ? "DONT_CAR"
                   : (sprintf(str, "%d", PD_EEProm->PDO2_GPIO[i]), str));
        printf("\tPDO3_GPIO[%d]     : %s \n",
               i,
               PD_EEProm->PDO3_GPIO[i] == GPIO_DONT_CARE
                   ? "DONT_CAR"
                   : (sprintf(str, "%d", PD_EEProm->PDO3_GPIO[i]), str));
        printf("\tPDO4_GPIO[%d]     : %s \n",
               i,
               PD_EEProm->PDO4_GPIO[i] == GPIO_DONT_CARE
                   ? "DONT_CAR"
                   : (sprintf(str, "%d", PD_EEProm->PDO4_GPIO[i]), str));
        printf("\tPDO5_GPIO[%d]     : %s \n",
               i,
               PD_EEProm->PDO5_GPIO[i] == GPIO_DONT_CARE
                   ? "DONT_CAR"
                   : (sprintf(str, "%d", PD_EEProm->PDO5_GPIO[i]), str));
        printf("\tPDO6_GPIO[%d]     : %s \n",
               i,
               PD_EEProm->PDO6_GPIO[i] == GPIO_DONT_CARE
                   ? "DONT_CAR"
                   : (sprintf(str, "%d", PD_EEProm->PDO6_GPIO[i]), str));
        printf("\tPDO7_GPIO[%d]     : %s \n",
               i,
               PD_EEProm->PDO7_GPIO[i] == GPIO_DONT_CARE
                   ? "DONT_CAR"
                   : (sprintf(str, "%d", PD_EEProm->PDO7_GPIO[i]), str));
        printf("\tVSET0V_GPIO[%d]   : %s \n",
               i,
               PD_EEProm->VSET0V_GPIO[i] == GPIO_DONT_CARE
                   ? "DONT_CAR"
                   : (sprintf(str, "%d", PD_EEProm->VSET0V_GPIO[i]), str));
        printf("\tVSAFE5V_GPIO[%d]  : %s \n",
               i,
               PD_EEProm->VSAFE5V_GPIO[i] == GPIO_DONT_CARE
                   ? "DONT_CAR"
                   : (sprintf(str, "%d", PD_EEProm->VSAFE5V_GPIO[i]), str));
        printf("\n\n");
    }
