...
#define LIBUSB_OPTION_WEAK_AUTHORITY LIBUSB_OPTION_NO_DEVICE_DISCOVERY
    ...
    void LIBUSB_CALL libusb_free_ss_usb_device_capability_descriptor(
        struct libusb_ss_usb_device_capability_descriptor *ss_usb_device_cap);
    int LIBUSB_CALL
    libusb_get_container_id_descriptor(libusb_context                              *ctx,
                                       struct libusb_bos_dev_capability_descriptor *dev_cap,
    ...
    void LIBUSB_CALL
    libusb_free_platform_descriptor(struct libusb_platform_descriptor *platform_descriptor);
    ...
    int LIBUSB_CALL libusb_get_active_interface_association_descriptors(
        libusb_device *dev, struct libusb_interface_association_descriptor_array **iad_array);
    void LIBUSB_CALL libusb_free_interface_association_descriptors(
        struct libusb_interface_association_descriptor_array *iad_array);

    int LIBUSB_CALL            libusb_wrap_sys_device(libusb_context        *ctx,
                                                      intptr_t               sys_dev,
                                                      libusb_device_handle **dev_handle);
    ...
    libusb_get_iso_packet_buffer_simple(struct libusb_transfer *transfer, unsigned int packet)
    {
        int _packet;
    ...
    typedef enum
    {
         ...
         * It is the user's responsibility to call libusb_close on any handle associated with a
         * disconnected device. It is safe to call libusb_get_device_descriptor on a device that has
         * left */
...
#define LIBUSB_HOTPLUG_MATCH_ANY -1
     ...
     * \param[out] callback_handle pointer to store the handle of the allocated callback (can be
     * NULL) \returns \ref LIBUSB_SUCCESS on success LIBUSB_ERROR code on failure
     */
    int LIBUSB_CALL
