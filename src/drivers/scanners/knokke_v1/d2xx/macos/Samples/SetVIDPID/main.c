...
int main(void)
{
     ...
     */
    ftStatus = FT_ListDevices(&standardDevices, NULL, FT_LIST_NUMBER_ONLY);
    if (ftStatus != FT_OK)
    {
    ...
    ftStatus = FT_ListDevices(&totalDevices, NULL, FT_LIST_NUMBER_ONLY);
    if (ftStatus != FT_OK)
    {
        printf("FT_ListDevices failed: error code %d.\n", (int)ftStatus);
        goto exit;
    ...
    printf("%d supported device%s detected: %d standard, %d custom.\n",
           (int)totalDevices,
           totalDevices == 1 ? "" : "s",
           (int)standardDevices,
