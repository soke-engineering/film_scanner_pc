...
int usbi_parse_descriptor(const unsigned char *source,
                          ...
                          int                                host_endian)
{
    ...
    {
        usbi_warn(ctx, "short endpoint descriptor read %d/%d", size, header.bLength);
        return parsed;
    }
    ...
    while (size >= DESC_HEADER_LENGTH)
    {
        ...
        {
            usbi_err(ctx, "invalid extra ep desc len (%d)", header.bLength);
            return LIBUSB_ERROR_IO;
        }
        else if (header.bLength > size)
        {
            usbi_warn(ctx, "short extra ep desc read %d/%d", size, header.bLength);
            return parsed;
        }
                           ...
                           int                      host_endian)
{
    ...
    while (size >= INTERFACE_DESC_LENGTH)
    {
        ...
        {
            usbi_err(ctx, "invalid interface bLength (%d)", ifp->bLength);
            r = LIBUSB_ERROR_IO;
            goto err;
        }
        if (ifp->bLength > size)
        {
            usbi_warn(ctx, "short intf descriptor read %d/%d", size, ifp->bLength);
            return parsed;
        }
        ...
        while (size >= DESC_HEADER_LENGTH)
        {
            ...
            if (header.bLength < DESC_HEADER_LENGTH)
            {
                usbi_err(ctx, "invalid extra intf desc len (%d)", header.bLength);
                r = LIBUSB_ERROR_IO;
                goto err;
            }
            else if (header.bLength > size)
            {
                usbi_warn(ctx, "short extra intf desc read %d/%d", size, header.bLength);
                return parsed;
            }
        ...
        if (size < LIBUSB_DT_INTERFACE_SIZE || ifp->bDescriptorType != LIBUSB_DT_INTERFACE ||
                               ...
                               int                              host_endian)
{
    ...
    {
        usbi_err(ctx, "short config descriptor read %d/%d", size, config->bLength);
        return LIBUSB_ERROR_IO;
    }
    ...
    for (i = 0; i < config->bNumInterfaces; i++)
    {
        ...
        while (size >= DESC_HEADER_LENGTH)
        {
            ...
            if (header.bLength < DESC_HEADER_LENGTH)
            {
                usbi_err(ctx, "invalid extra config desc len (%d)", header.bLength);
                r = LIBUSB_ERROR_IO;
                goto err;
            }
            else if (header.bLength > size)
            {
                usbi_warn(ctx, "short extra config desc read %d/%d", size, header.bLength);
...
int usbi_device_cache_descriptor(libusb_device *dev)
{
    ...
    r = usbi_backend.get_device_descriptor(
        dev, (unsigned char *)&dev->device_descriptor, &host_endian);
    if (r < 0)
        return r;
                                              ...
                                              struct libusb_device_descriptor *desc)
{
    ...
    memcpy((unsigned char *)desc,
           (unsigned char *)&dev->device_descriptor,
                                              ...
                                              struct libusb_config_descriptor **config)
{
    ...
    r = usbi_backend.get_config_descriptor(
        dev, config_index, tmp, LIBUSB_DT_CONFIG_SIZE, &host_endian);
    if (r < 0)
        return r;
    if (r < LIBUSB_DT_CONFIG_SIZE)
    {
    ...
    r = usbi_backend.get_config_descriptor(
        dev, config_index, buf, _config.wTotalLength, &host_endian);
    if (r >= 0)
        r = raw_desc_to_config(dev->ctx, buf, r, host_endian, config);
...
int usbi_get_config_index_by_value(struct libusb_device *dev, uint8_t bConfigurationValue, int *idx)
{
    ...
    for (i = 0; i < dev->num_configurations; i++)
    {
        unsigned char tmp[6];
        int           host_endian;
        int r = usbi_backend.get_config_descriptor(dev, i, tmp, sizeof(tmp), &host_endian);
        if (r < 0)
        {
            *idx = -1;
            return r;
                                                       ...
                                                       struct libusb_config_descriptor **config)
{
    ...
    {
        r = usbi_backend.get_config_descriptor_by_value(
            dev, bConfigurationValue, &buf, &host_endian);
        if (r < 0)
            return r;
    ...
    struct libusb_ss_endpoint_companion_descriptor **ep_comp)
{
    ...
    *ep_comp = NULL;
    ...
    while (size >= DESC_HEADER_LENGTH)
    {
        ...
        {
            usbi_err(ctx, "invalid ss-ep-comp-desc length %d", header.bLength);
            return LIBUSB_ERROR_IO;
        }
                     ...
                     int                            host_endian)
{
    ...
    {
        usbi_err(ctx, "short bos descriptor read %d/%d", size, bos_header.bLength);
        return LIBUSB_ERROR_IO;
    }
    ...
    for (i = 0; i < bos_header.bNumDeviceCaps; i++)
    {
        ...
        {
            usbi_warn(
                ctx, "short dev-cap descriptor read %d/%d", size, LIBUSB_DT_DEVICE_CAPABILITY_SIZE);
            break;
        }
        ...
        {
            usbi_err(ctx, "invalid dev-cap bLength (%d)", dev_cap.bLength);
            libusb_free_bos_descriptor(_bos);
            return LIBUSB_ERROR_IO;
        }
        if (dev_cap.bLength > size)
        {
            usbi_warn(ctx, "short dev-cap descriptor read %d/%d", size, dev_cap.bLength);
            break;
        }
                                           ...
                                           struct libusb_bos_descriptor **bos)
{
    ...
    {
        usbi_err(HANDLE_CTX(dev_handle), "short BOS read %d/%d", r, LIBUSB_DT_BOS_SIZE);
        return LIBUSB_ERROR_IO;
    }
    ...
    struct libusb_usb_2_0_extension_descriptor **usb_2_0_extension)
{
    ...
    usbi_parse_descriptor((unsigned char *)dev_cap, "bbbd", _usb_2_0_extension, host_endian);
    ...
    struct libusb_ss_usb_device_capability_descriptor **ss_usb_device_cap)
{
    ...
    usbi_parse_descriptor((unsigned char *)dev_cap, "bbbbwbbw", _ss_usb_device_cap, host_endian);
                                   ...
                                   struct libusb_container_id_descriptor      **container_id)
{
    ...
    usbi_parse_descriptor((unsigned char *)dev_cap, "bbbbu", _container_id, host_endian);
                                                    ...
                                                    int                   length)
{
    ...
    r = libusb_get_string_descriptor(dev_handle, desc_index, langid, tbuf, sizeof(tbuf));
    if (r < 0)
        return r;
