...
#define DARWIN_CACHED_DEVICE(a) \
    ((struct darwin_cached_device *)(((struct darwin_device_priv *)((a)->os_priv))->dev))
...
static int ep_to_pipeRef(struct libusb_device_handle *dev_handle,
                         uint8_t                      ep,
                         uint8_t                     *pipep,
                         uint8_t                     *ifcp,
                         struct darwin_interface    **interface_out)
{
...
static int usb_setup_device_iterator(io_iterator_t *deviceIterator, UInt32 location)
{
    ...
    if (location)
    {
        ...
        /* there are no unsigned CFNumber types so treat the value as signed. the OS seems to do
           this internally (CFNumberType of locationID is kCFNumberSInt32Type) */
        CFTypeRef locationCF = CFNumberCreate(NULL, kCFNumberSInt32Type, &location);

        if (propertyMatchDict && locationCF)
        {
            CFDictionarySetValue(
                propertyMatchDict, CFSTR(kUSBDevicePropertyLocationID), locationCF);
            CFDictionarySetValue(matchingDict, CFSTR(kIOPropertyMatchKey), propertyMatchDict);
        }
        /* else we can still proceed as long as the caller accounts for the possibility of other
         * devices in the iterator */
...
static int
get_ioregistry_value_number(io_service_t service, CFStringRef property, CFNumberType type, void *p)
{
...
static int
get_ioregistry_value_data(io_service_t service, CFStringRef property, ssize_t size, void *p)
{
...
static usb_device_t **darwin_device_from_service(io_service_t service)
{
    ...
    (void)(*plugInInterface)
        ->QueryInterface(plugInInterface, CFUUIDGetUUIDBytes(DeviceInterfaceID), (LPVOID)&device);
    /* Use release instead of IODestroyPlugInInterface to avoid stopping IOServices associated with
     * this device */
...
static void darwin_devices_detached(void *ptr, io_iterator_t rem_devices)
{
    ...
    while ((device = IOIteratorNext(rem_devices)) != 0)
    {
        ...
        ret =
            get_ioregistry_value_number(device, CFSTR("sessionID"), kCFNumberSInt64Type, &session);
        ...
        list_for_each_entry(ctx, &active_contexts_list, list, struct libusb_context)
        {
            ...
            if (dev)
            {
                /* signal the core that this device has been disconnected. the core will tear down
                   this device when the reference count reaches 0 */
...
static void *darwin_event_thread_main(void *arg0)
{
    ...
    kresult = IOServiceAddMatchingNotification(libusb_notification_port,
...
static int
darwin_get_device_descriptor(struct libusb_device *dev, unsigned char *buffer, int *host_endian)
{
...
static int darwin_check_configuration(struct libusb_context *ctx, struct darwin_cached_device *dev)
{
    ...
    /* check if the device is already configured. there is probably a better way than iterating over
       the to accomplish this (the trick is we need to avoid a call to GetConfigurations since buggy
       devices might lock up on the device request) */
    ...
    if (firstInterface)
    {
        ...
        if (dev->dev_descriptor.bNumConfigurations == 1)
            /* to avoid problems with some devices get the configurations value from the
...
static int darwin_request_descriptor(
    usb_device_t **device, UInt8 desc, UInt8 desc_index, void *buffer, size_t buffer_size)
{
                                          ...
                                          struct darwin_cached_device *dev)
{
    ...
    /* According to Apple's documentation the device must be open for DeviceRequest but we may not
     * be able to open some devices and Apple's USB Prober doesn't bother to open the device before
     * issuing a descriptor request.  Still, to follow the spec as closely as possible, try opening
     * the device */
    ...
    do
    {
        ...
        ret = darwin_request_descriptor(
            device, kUSBDeviceDesc, 0, &dev->dev_descriptor, sizeof(dev->dev_descriptor));
        ...
        if (kIOReturnSuccess == ret &&
            (0 == dev->dev_descriptor.bNumConfigurations || 0 == dev->dev_descriptor.bcdUSB))
        {
            /* work around for incorrectly configured devices */
            if (try_reconfigure && is_open)
            {
                usbi_dbg("descriptor appears to be invalid. resetting configuration before trying "
                         "again...");
        ...
        if (kIOReturnSuccess != ret && is_open && try_unsuspend)
        {
            ...
            /* IOUSBFamily 320+ provides a way to detect device suspension but earlier versions do
             * not */
        ...
        if (kIOReturnSuccess != ret)
        {
            usbi_dbg("kernel responded with code: 0x%08x. sleeping for %d ms before trying again",
                     ret,
                     delay / 1000);
    ...
    if (ret != kIOReturnSuccess)
    {
        ...
        if (LIBUSB_CLASS_HUB == bDeviceClass)
            ...
            usbi_warn(ctx,
                      "could not retrieve device descriptor %.4x:%.4x: %s (%x). skipping device",
                      idVendor,
                      idProduct,
                      darwin_error_str(ret),
                      ret);
    ...
    /* catch buggy hubs (which appear to be virtual). Apple's own USB prober has problems with these
     * devices. */
    if (libusb_le16_to_cpu(dev->dev_descriptor.idProduct) != idProduct)
    {
        /* not a valid device */
        usbi_warn(ctx,
                  "idProduct from iokit (%04x) does not match idProduct in descriptor (%04x). "
                  "skipping device",
                  idProduct,
...
static int get_device_parent_sessionID(io_service_t service, UInt64 *parent_sessionID)
{
    ...
    parent = service;
    while ((result = IORegistryEntryGetParentEntry(parent, kIOServicePlane, &parent)) ==
           kIOReturnSuccess)
    {
...
static int darwin_get_cached_device(struct libusb_context        *ctx,
                                    io_service_t                  service,
                                    struct darwin_cached_device **cached_out)
{
    ...
    do
    {
        *cached_out = NULL;
        ...
        {
            usbi_dbg("matching sessionID 0x%" PRIx64
                     " against cached device with sessionID 0x%" PRIx64,
                     sessionID,
                     new_device->session);
        ...
        {
            snprintf(new_device->sys_path,
                     20,
                     "%03i-%04x-%04x-%02x-%02x",
                     new_device->address,
                     new_device->dev_descriptor.idVendor,
                     new_device->dev_descriptor.idProduct,
...
static int process_new_device(struct libusb_context *ctx, io_service_t service)
{
    ...
    do
    {
        ...
        {
            dev->parent_dev =
                usbi_get_device_by_session_id(ctx, (unsigned long)cached_device->parent_session);
        }
        else
        {
            dev->parent_dev = NULL;
        ...
        usbi_dbg("found device with address %d port = %d parent = %p at %p",
                 dev->device_address,
                 dev->port_number,
                 (void *)dev->parent_dev,
                 priv->dev->sys_path);
...
static int darwin_open(struct libusb_device_handle *dev_handle)
{
    ...
    if (0 == dpriv->open_count)
    {
        ...
        if (kresult != kIOReturnSuccess)
        {
            ...
            /* it is possible to perform some actions on a device that is not open so do not return
             * an error */
            priv->is_open = 0;
        }
        else
        ...
        if (kresult != kIOReturnSuccess)
        {
            usbi_err(HANDLE_CTX(dev_handle),
                     "CreateDeviceAsyncEventSource: %s",
...
static int
darwin_get_interface(usb_device_t **darwin_device, uint8_t ifc, io_service_t *usbInterfacep)
{
    IOUSBFindInterfaceRequest request;
    kern_return_t             kresult;
    io_iterator_t             interface_iterator;
    ...
    *usbInterfacep = IO_OBJECT_NULL;
    ...
    while ((*usbInterfacep = IOIteratorNext(interface_iterator)))
    {
        ...
        ret = get_ioregistry_value_number(
            *usbInterfacep, CFSTR("bInterfaceNumber"), kCFNumberSInt8Type, &bInterfaceNumber);
...
static int get_endpoints(struct libusb_device_handle *dev_handle, int iface)
{
    ...
    for (int i = 1; i <= numep; i++)
    {
        kresult = (*(cInterface->interface))
                      ->GetPipeProperties(cInterface->interface,
                                          i,
                                          &direction,
                                          &number,
                                          &dont_care1,
        ...
        if (kresult != kIOReturnSuccess)
        {
            ...
            kresult = (*(cInterface->interface))
                          ->GetAlternateSetting(cInterface->interface, &alt_setting);
            if (kresult)
            {
        ...
        {
            cInterface->endpoint_addrs[i - 1] = (((kUSBIn == direction) << kUSBRqDirnShift) |
                                                 (number & LIBUSB_ENDPOINT_ADDRESS_MASK));
        }

        usbi_dbg("interface: %i pipe %i: dir: %i number: %i",
                 iface,
                 i,
                 cInterface->endpoint_addrs[i - 1] >> kUSBRqDirnShift,
...
static int darwin_claim_interface(struct libusb_device_handle *dev_handle, int iface)
{
    ...
    {
        usbi_err(HANDLE_CTX(dev_handle),
                 "IOCreatePlugInInterfaceForService: %s",
                 darwin_error_str(kresult));
        return darwin_to_libusb(kresult);
    }
    ...
    /* create async event source */
    kresult = (*(cInterface->interface))
                  ->CreateInterfaceAsyncEventSource(cInterface->interface, &cInterface->cfSource);
    if (kresult != kIOReturnSuccess)
    {
...
static int
darwin_set_interface_altsetting(struct libusb_device_handle *dev_handle, int iface, int altsetting)
{
    struct darwin_device_handle_priv *priv =
        (struct darwin_device_handle_priv *)dev_handle->os_priv;
    IOReturn kresult;
...
static int darwin_reset_device(struct libusb_device_handle *dev_handle)
{
    ...
    do
    {
        ...
        /* ignore return code. if we can't get a descriptor it might be worthwhile re-enumerating
         * anway */
        (void)darwin_request_descriptor(
            dpriv->device, kUSBDeviceDesc, 0, &descriptor, sizeof(descriptor));
        ...
        for (i = 0; i < descriptor.bNumConfigurations; ++i)
        {
            ...
            (void)darwin_request_descriptor(
                dpriv->device, kUSBConfDesc, i, &configuration, sizeof(configuration));
            (*(dpriv->device))
                ->GetConfigurationDescriptorPtr(dpriv->device, i, &cached_configuration);

            if (!cached_configuration ||
                0 != memcmp(cached_configuration, &configuration, sizeof(configuration)))
            {
                reenumerate = true;
                break;
            }
...
static int darwin_kernel_driver_active(struct libusb_device_handle *dev_handle, int interface)
{
    ...
    driver = IORegistryEntryCreateCFProperty(
        usbInterface, kIOBundleIdentifierKey, kCFAllocatorDefault, 0);
...
static int submit_bulk_transfer(struct usbi_transfer *itransfer)
{
    ...
    if (transferType == kUSBInterrupt)
    {
        if (IS_XFERIN(transfer))
            ret = (*(cInterface->interface))
                      ->ReadPipeAsync(cInterface->interface,
                                      pipeRef,
                                      transfer->buffer,
                                      transfer->length,
            ...
            ret = (*(cInterface->interface))
                      ->WritePipeAsync(cInterface->interface,
                                       pipeRef,
                                       transfer->buffer,
                                       transfer->length,
                                       darwin_async_io_callback,
    ...
    else
    {
        ...
        if (IS_XFERIN(transfer))
            ret = (*(cInterface->interface))
                      ->ReadPipeAsyncTO(cInterface->interface,
                                        pipeRef,
                                        transfer->buffer,
                                        transfer->length,
                                        transfer->timeout,
            ...
            ret = (*(cInterface->interface))
                      ->WritePipeAsyncTO(cInterface->interface,
                                         pipeRef,
                                         transfer->buffer,
                                         transfer->length,
                                         transfer->timeout,
...
static int submit_stream_transfer(struct usbi_transfer *itransfer)
{
    ...
    if (IS_XFERIN(transfer))
        ret = (*(cInterface->interface))
                  ->ReadStreamsPipeAsyncTO(cInterface->interface,
                                           pipeRef,
                                           itransfer->stream_id,
                                           transfer->buffer,
                                           transfer->length,
        ...
        ret = (*(cInterface->interface))
                  ->WriteStreamsPipeAsyncTO(cInterface->interface,
                                            pipeRef,
                                            itransfer->stream_id,
                                            transfer->buffer,
                                            transfer->length,
    ...
    if (ret)
        usbi_err(TRANSFER_CTX(transfer),
                 "bulk stream transfer failed (dir = %s): %s (code = 0x%08x)",
                 IS_XFERIN(transfer) ? "In" : "Out",
                 darwin_error_str(ret),
                 ret);
...
static int submit_iso_transfer(struct usbi_transfer *itransfer)
{
    ...
    if (!tpriv->isoc_framelist)
    {
        ...
        tpriv->isoc_framelist =
            (IOUSBIsocFrame *)calloc(transfer->num_iso_packets, sizeof(IOUSBIsocFrame));
        if (!tpriv->isoc_framelist)
            return LIBUSB_ERROR_NO_MEM;
    }
    ...
    if (IS_XFERIN(transfer))
        kresult = (*(cInterface->interface))
                      ->ReadIsochPipeAsync(cInterface->interface,
                                           pipeRef,
                                           transfer->buffer,
                                           frame,
        ...
        kresult = (*(cInterface->interface))
                      ->WriteIsochPipeAsync(cInterface->interface,
                                            pipeRef,
                                            transfer->buffer,
                                            frame,
                                            transfer->num_iso_packets,
    ...
    if (LIBUSB_SPEED_FULL == transfer->dev_handle->dev->speed)
        ...
        cInterface->frames[transfer->endpoint] =
            frame + transfer->num_iso_packets * (1 << (interval - 1));
    else
        /* High/super speed */
        cInterface->frames[transfer->endpoint] =
            frame + transfer->num_iso_packets * (1 << (interval - 1)) / 8;
...
static int submit_control_transfer(struct usbi_transfer *itransfer)
{
    ...
    if (transfer->endpoint)
    {
        ...
        if (ep_to_pipeRef(transfer->dev_handle, transfer->endpoint, &pipeRef, NULL, &cInterface) !=
            0)
        {
            usbi_err(TRANSFER_CTX(transfer), "endpoint not found on any open interface");
        ...
        kresult = (*(cInterface->interface))
                      ->ControlRequestAsyncTO(cInterface->interface,
                                              pipeRef,
                                              &(tpriv->req),
                                              darwin_async_io_callback,
                                              itransfer);
        ...
        kresult = (*(dpriv->device))
                      ->DeviceRequestAsyncTO(
                          dpriv->device, &(tpriv->req), darwin_async_io_callback, itransfer);
...
#else
        usbi_err(TRANSFER_CTX(transfer),
                 "IOUSBFamily version does not support bulk stream transfers");
        return LIBUSB_ERROR_NOT_SUPPORTED;
#endif
    default:
...
static int darwin_abort_transfers(struct usbi_transfer *itransfer)
{
    ...
    usbi_warn(ITRANSFER_CTX(itransfer),
              "aborting all transactions on interface %d pipe %d",
              iface,
              pipeRef);
    ...
    if (LIBUSB_TRANSFER_TYPE_BULK_STREAM == transfer->type)
        (*(cInterface->interface))
            ->AbortStreamsPipe(cInterface->interface, pipeRef, itransfer->stream_id);
    else
#endif
...
static void darwin_async_io_callback(void *refcon, IOReturn result, void *arg0)
{
    ...
    if (kIOReturnSuccess == result && IS_XFEROUT(transfer) &&
        transfer->flags & LIBUSB_TRANSFER_ADD_ZERO_PACKET)
    {
        struct darwin_interface *cInterface;
        uint8_t                  pipeRef;
...
static int darwin_handle_transfer_completion(struct usbi_transfer *itransfer)
{
    ...
    /* it is ok to handle cancelled transfers without calling usbi_handle_transfer_cancellation (we
     * catch timeout transfers) */
    return usbi_handle_transfer_completion(itransfer,
                                           darwin_transfer_status(itransfer, tpriv->result));
}
...
#if InterfaceVersion >= 550
static int darwin_alloc_streams(struct libusb_device_handle *dev_handle,
                                ...
                                int                          num_endpoints)
{
    ...
    for (i = 0; i < num_endpoints; ++i)
    {
        ...
        (*(cInterface->interface))
            ->SupportsStreams(cInterface->interface, pipeRef, &supportsStreams);
...
static int darwin_free_streams(struct libusb_device_handle *dev_handle,
                               unsigned char               *endpoints,
                               int                          num_endpoints)
{
    struct darwin_interface *cInterface;
