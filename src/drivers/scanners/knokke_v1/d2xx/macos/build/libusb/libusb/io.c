...
{
    event_thread_run = 0;
    libusb_hotplug_deregister_callback(ctx, hotplug_cb_handle); // This wakes up
libusb_handle_events() pthread_join(event_thread); libusb_exit(ctx);
}
\endcode
 */
...
#ifdef USBI_TIMERFD_AVAILABLE
    ctx->timerfd = timerfd_create(usbi_backend.get_timerfd_clockid(), TFD_NONBLOCK | TFD_CLOEXEC);
    if (ctx->timerfd >= 0)
    {
...
static int calculate_timeout(struct usbi_transfer *transfer)
{
    ...
    if (r < 0)
    {
        usbi_err(ITRANSFER_CTX(transfer), "failed to read monotonic clock, errno=%d", errno);
        return r;
    }
...
struct libusb_transfer *LIBUSB_CALL libusb_alloc_transfer(int iso_packets)
{
    ...
    size_t alloc_size = sizeof(struct usbi_transfer) + sizeof(struct libusb_transfer) +
                        (sizeof(struct libusb_iso_packet_descriptor) * iso_packets) + os_alloc_size;
...
static int add_to_flying_list(struct usbi_transfer *transfer)
{
    ...
    list_for_each_entry(cur, &ctx->flying_transfers, list, struct usbi_transfer)
    {
        ...
        if (!timerisset(cur_tv) || (cur_tv->tv_sec > timeout->tv_sec) ||
            (cur_tv->tv_sec == timeout->tv_sec && cur_tv->tv_usec > timeout->tv_usec))
        {
 ...
 */
void API_EXPORTED libusb_transfer_set_stream_id(struct libusb_transfer *transfer,
                                                uint32_t                stream_id)
{
    struct usbi_transfer *itransfer = LIBUSB_TRANSFER_TO_USBI_TRANSFER(transfer);
...
static void handle_timeout(struct usbi_transfer *itransfer)
{
    ...
    if (r == LIBUSB_SUCCESS)
        ...
        usbi_warn(TRANSFER_CTX(transfer), "async cancel failed %d errno=%d", r, errno);
}
...
void usbi_remove_pollfd(struct libusb_context *ctx, int fd)
{
    ...
    usbi_mutex_lock(&ctx->event_data_lock);
    list_for_each_entry(ipollfd, &ctx->ipollfds, list, struct usbi_pollfd) if (ipollfd->pollfd.fd ==
                                                                               fd)
    {
        found = 1;
        break;
...
const struct libusb_pollfd **LIBUSB_CALL libusb_get_pollfds(libusb_context *ctx)
{
    ...
    list_for_each_entry(ipollfd, &ctx->ipollfds, list, struct usbi_pollfd) ret[i++] =
        (struct libusb_pollfd *)ipollfd;
...
#else
    usbi_err(ctx,
             "external polling of libusb's internal descriptors "
...
void usbi_handle_disconnect(struct libusb_device_handle *dev_handle)
{
    ...
    usbi_dbg("device %d.%d", dev_handle->dev->bus_number, dev_handle->dev->device_address);
    ...
    while (1)
    {
        ...
        list_for_each_entry(
            cur,
            &HANDLE_CTX(dev_handle)->flying_transfers,
            list,
            struct usbi_transfer) if (USBI_TRANSFER_TO_LIBUSB_TRANSFER(cur)->dev_handle ==
                                      dev_handle)
        {
