...
static int _get_usbfs_fd(struct libusb_device *dev, mode_t mode, int silent)
{
    ...
    if (usbdev_names)
        snprintf(
            path, PATH_MAX, "%s/usbdev%d.%d", usbfs_path, dev->bus_number, dev->device_address);
    else
    ...
    if (!silent)
    {
        ...
        if (errno == EACCES && mode == O_RDWR)
            usbi_err(ctx,
                     "libusb requires write access to USB "
                     "device nodes.");
    }
...
static int _open_sysfs_attr(struct libusb_device *dev, const char *attr)
{
    ...
    snprintf(filename, PATH_MAX, "%s/%s/%s", SYSFS_DEVICE_PATH, priv->sysfs_dir, attr);
    fd = _open(filename, O_RDONLY);
    if (fd < 0)
    {
        usbi_err(DEVICE_CTX(dev), "open %s failed ret=%d errno=%d", filename, fd, errno);
        return LIBUSB_ERROR_IO;
    }
...
static int __read_sysfs_attr(struct libusb_context *ctx, const char *devname, const char *attr)
{
    char  filename[PATH_MAX];
    FILE *f;
    int   fd, r, value;
...
static int
op_get_device_descriptor(struct libusb_device *dev, unsigned char *buffer, int *host_endian)
{
    struct linux_device_priv *priv = _device_priv(dev);
...
static int sysfs_get_active_config(struct libusb_device *dev, int *config)
{
    ...
    if (r < 0)
    {
        usbi_err(DEVICE_CTX(dev), "read bConfigurationValue failed ret=%d errno=%d", r, errno);
        return LIBUSB_ERROR_IO;
    }
    else if (r == 0)
    {
...
static int seek_to_next_config(struct libusb_context *ctx, unsigned char *buffer, int size)
{
    ...
    {
        usbi_err(ctx, "short descriptor read %d/%d", size, LIBUSB_DT_CONFIG_SIZE);
        return LIBUSB_ERROR_IO;
    }
...
static int
initialize_device(struct libusb_device *dev, uint8_t busnum, uint8_t devaddr, const char *sysfs_dir)
{
    struct linux_device_priv *priv             = _device_priv(dev);
    struct libusb_context    *ctx              = DEVICE_CTX(dev);
    ...
    do
    {
        ...
        r = read(fd,
                 priv->descriptors + priv->descriptors_len,
                 descriptors_size - priv->descriptors_len);
        if (r < 0)
        {
...
retry:
    ...
    usbi_dbg("Dev %p (%s) has parent %p (%s) port %d",
             dev,
             sysfs_dir,
             dev->parent_dev,
             parent_sysfs_dir,
...
static int release_interface(struct libusb_device_handle *handle, int iface)
{
    ...
    if (r)
    {
        ...
        usbi_err(HANDLE_CTX(handle), "release interface failed, error %d errno %d", r, errno);
        return LIBUSB_ERROR_OTHER;
    }
    return 0;
}
...
static int op_set_interface(struct libusb_device_handle *handle, int iface, int altsetting)
{
    ...
    if (r)
    {
        ...
        usbi_err(HANDLE_CTX(handle), "setintf failed error %d errno %d", r, errno);
        return LIBUSB_ERROR_OTHER;
    }
...
static int op_clear_halt(struct libusb_device_handle *handle, unsigned char endpoint)
{
    ...
    if (r)
    {
        ...
        usbi_err(HANDLE_CTX(handle), "clear_halt failed error %d errno %d", r, errno);
        return LIBUSB_ERROR_OTHER;
    }
...
static int op_reset_device(struct libusb_device_handle *handle)
{
    ...
    if (r)
    {
        ...
        usbi_err(HANDLE_CTX(handle), "reset failed error %d errno %d", r, errno);
                            ...
                            int                          num_endpoints)
{
    ...
    if (r < 0)
    {
        ...
        usbi_err(HANDLE_CTX(handle), "streams-ioctl failed error %d errno %d", r, errno);
        return LIBUSB_ERROR_OTHER;
    }
    return r;
}
                            ...
                            int                          num_endpoints)
{
    return do_streams_ioctl(
        handle, IOCTL_USBFS_ALLOC_STREAMS, num_streams, endpoints, num_endpoints);
}

static int
op_free_streams(struct libusb_device_handle *handle, unsigned char *endpoints, int num_endpoints)
{
    return do_streams_ioctl(handle, IOCTL_USBFS_FREE_STREAMS, 0, endpoints, num_endpoints);
}

static unsigned char *op_dev_mem_alloc(struct libusb_device_handle *handle, size_t len)
{
    struct linux_device_handle_priv *hpriv = _device_handle_priv(handle);
    unsigned char                   *buffer =
        (unsigned char *)mmap(NULL, len, PROT_READ | PROT_WRITE, MAP_SHARED, hpriv->fd, 0);
...
static int op_dev_mem_free(struct libusb_device_handle *handle, unsigned char *buffer, size_t len)
{
...
static int op_attach_kernel_driver(struct libusb_device_handle *handle, int interface)
{
    ...
    if (r < 0)
    {
        ...
        usbi_err(HANDLE_CTX(handle), "attach failed error %d errno %d", r, errno);
        return LIBUSB_ERROR_OTHER;
    }
    else if (r == 0)
    {
...
static int
op_handle_events(struct libusb_context *ctx, struct pollfd *fds, POLL_NFDS_TYPE nfds, int num_ready)
{
    int          r;
    unsigned int i = 0;
...
static clockid_t op_get_timerfd_clockid(void) { return monotonic_clkid; }
#endif
