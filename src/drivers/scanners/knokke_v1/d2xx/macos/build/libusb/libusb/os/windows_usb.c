...
// Common calls
static int
common_configure_endpoints(int sub_api, struct libusb_device_handle *dev_handle, int iface);
...
static int
winusbx_configure_endpoints(int sub_api, struct libusb_device_handle *dev_handle, int iface);
static int winusbx_claim_interface(int sub_api, struct libusb_device_handle *dev_handle, int iface);
static int
winusbx_release_interface(int sub_api, struct libusb_device_handle *dev_handle, int iface);
static int winusbx_submit_control_transfer(int sub_api, struct usbi_transfer *itransfer);
static int winusbx_set_interface_altsetting(int                          sub_api,
                                            struct libusb_device_handle *dev_handle,
                                            int                          iface,
                                            int                          altsetting);
static int winusbx_submit_bulk_transfer(int sub_api, struct usbi_transfer *itransfer);
static int
winusbx_clear_halt(int sub_api, struct libusb_device_handle *dev_handle, unsigned char endpoint);
static int winusbx_abort_transfers(int sub_api, struct usbi_transfer *itransfer);
static int winusbx_abort_control(int sub_api, struct usbi_transfer *itransfer);
static int winusbx_reset_device(int sub_api, struct libusb_device_handle *dev_handle);
static int
winusbx_copy_transfer_data(int sub_api, struct usbi_transfer *itransfer, uint32_t io_size);
// HID API prototypes
...
static int
hid_clear_halt(int sub_api, struct libusb_device_handle *dev_handle, unsigned char endpoint);
...
static int
composite_claim_interface(int sub_api, struct libusb_device_handle *dev_handle, int iface);
static int composite_set_interface_altsetting(int                          sub_api,
                                              struct libusb_device_handle *dev_handle,
                                              int                          iface,
                                              int                          altsetting);
static int
composite_release_interface(int sub_api, struct libusb_device_handle *dev_handle, int iface);
static int composite_submit_control_transfer(int sub_api, struct usbi_transfer *itransfer);
static int composite_submit_bulk_transfer(int sub_api, struct usbi_transfer *itransfer);
static int composite_submit_iso_transfer(int sub_api, struct usbi_transfer *itransfer);
static int
composite_clear_halt(int sub_api, struct libusb_device_handle *dev_handle, unsigned char endpoint);
...
static int
composite_copy_transfer_data(int sub_api, struct usbi_transfer *itransfer, uint32_t io_size);
...
static char *windows_error_str(uint32_t retval)
{
    ...
    size = FormatMessageA(FORMAT_MESSAGE_FROM_SYSTEM,
                          ...
                          ERR_BUFFER_SIZE - (DWORD)safe_strlen(err_string),
                          NULL);
    if (size == 0)
    {
    ...
    else
    {
        ...
        for (i = safe_strlen(err_string) - 1;
             (i >= 0) && ((err_string[i] == 0x0A) || (err_string[i] == 0x0D));
             i--)
        {
            err_string[i] = 0;
        }
                                                                ...
                                                                unsigned         _index)
{
    ...
    {
        usbi_err(ctx,
                 "program assertion failed - http://msdn.microsoft.com/en-us/library/ms792901.aspx "
                 "is wrong.");
        goto err_exit;
    }
    ...
    if (!pSetupDiGetDeviceInterfaceDetailA(
            *dev_info, &dev_interface_data, dev_interface_details, size, &size, NULL))
    {
        usbi_err(ctx,
...
/* For libusb0 filter */
static SP_DEVICE_INTERFACE_DETAIL_DATA_A *
get_interface_details_filter(struct libusb_context *ctx,
                             HDEVINFO              *dev_info,
                             SP_DEVINFO_DATA       *dev_info_data,
                             const GUID            *guid,
                             unsigned               _index,
                             char                  *filter_path)
{
    ...
    if (dev_interface_details)
    {
        ...
        if (hkey_device_interface != INVALID_HANDLE_VALUE)
        {
            ...
            DWORD value_type                 = 0;
            LONG  status;
            status = pRegQueryValueExW(hkey_device_interface,
                                       L"LUsb0",
                                       NULL,
                                       &value_type,
                                       (LPBYTE)&libusb0_symboliclink_index,
                                       &value_length);
            if (status == ERROR_SUCCESS)
            {
                if (libusb0_symboliclink_index < 256)
                {
                    ...
                    // If the the device interface guid is {F9F3FF14-AE21-48A0-8A25-8011A7A931D9}
                    // then it's a filter.
                ...
                {
                    // libusb0.sys was connected to this device instance at one time; but not
                    // anymore.
                }
            }
 ...
 */
static int
windows_assign_endpoints(struct libusb_device_handle *dev_handle, int iface, int altsetting)
{
    int                                       i, r;
    ...
    r = libusb_get_config_descriptor(
        dev_handle->dev, (uint8_t)(priv->active_config - 1), &conf_desc);
    if (r != LIBUSB_SUCCESS)
    {
    ...
    for (i = 0; i < if_desc->bNumEndpoints; i++)
    {
        ...
        usbi_dbg("(re)assigned endpoint %02X to interface %d",
                 priv->usb_interface[iface].endpoint[i],
                 iface);
    }
...
static void auto_release(struct usbi_transfer *itransfer)
{
    struct windows_transfer_priv *transfer_priv =
        (struct windows_transfer_priv *)usbi_transfer_get_os_priv(itransfer);
...
static void get_windows_version(void)
{
    ...
    if (vi.dwPlatformId == VER_PLATFORM_WIN32_NT)
    {
        ...
        {
            // Starting with Windows 8.1 Preview, GetVersionEx() does no longer report the actual OS
            // version See: http://msdn.microsoft.com/en-us/library/windows/desktop/dn302074.aspx
    ...
    if (!w)
        ...
        safe_sprintf(
            vptr, vlen, "%s SP%u.%u %s", w, vi.wServicePackMajor, vi.wServicePackMinor, w64);
...
static int windows_init(struct libusb_context *ctx)
{
    ...
    { // First init?
        ...
        if (QueryPerformanceFrequency(&li_frequency))
        {
            ...
            // calls, which we glue to the first available core always to prevent timing
            // discrepancies.
            ...
            timer_thread = (HANDLE)_beginthreadex(NULL,
                                                  0,
                                                  windows_clock_gettime_threaded,
                                                  (void *)event,
                                                  0,
                                                  (unsigned int *)&timer_thread_id);
...
static int cache_config_descriptors(struct libusb_device *dev, HANDLE hub_handle, char *device_id)
{
    ...
    priv->config_descriptor =
        (unsigned char **)calloc(dev->num_configurations, sizeof(unsigned char *));
    ...
    for (i = 0, r = LIBUSB_SUCCESS;; i++)
    {
        ...
        {
            usbi_info(ctx,
                      "could not access configuration descriptor (dummy) for '%s': %s",
                      device_id,
                      windows_error_str(0));
        ...
        if ((ret_size != size) ||
            (cd_buf_short.data.wTotalLength < sizeof(USB_CONFIGURATION_DESCRIPTOR)))
        {
        ...
        {
            usbi_err(
                ctx, "could not allocate configuration descriptor buffer for '%s'.", device_id);
        ...
        {
            usbi_err(ctx,
                     "could not access configuration descriptor (actual) for '%s': %s",
                     device_id,
                     windows_error_str(0));
            LOOP_BREAK(LIBUSB_ERROR_IO);
        }
        ...
        usbi_dbg("cached config descriptor %d (bConfigurationValue=%d, %d bytes)",
                 i,
                 cd_data->bConfigurationValue,
                 cd_data->wTotalLength);
                       ...
                       DWORD                 devinst)
{
    ...
    { // Not a HCD hub
        ...
        // In their great wisdom, Microsoft decided to BREAK the USB speed report between Windows 7
        // and Windows 8
                                   ...
                                   int                    sub_api)
{
    ...
    {
        if ((device_id[i++] == 'M') && (device_id[i++] == 'I') && (device_id[i++] == '_'))
        {
...
static int
set_hid_interface(struct libusb_context *ctx, struct libusb_device *dev, char *dev_interface_path)
{
    int                         i;
    struct windows_device_priv *priv = _device_priv(dev);
...
// #define ENUM_DEBUG
        ...
        for (i = 0;; i++)
        {
            ...
            {
                usbi_warn(ctx,
                          "program assertion failed - found more than %d buses, skipping the rest.",
                          UINT8_MAX);
                break;
            }
            if (pass != GEN_PASS)
            {
                ...
                dev_interface_details =
                    get_interface_details(ctx, &dev_info, &dev_info_data, guid[pass], i);
                ...
                else
                {
                    ...
                    {
                        usbi_warn(ctx,
                                  "could not sanitize device interface path for '%s'",
            ...
            case GEN_PASS:
                ...
                key = pSetupDiOpenDevRegKey(
                    dev_info, &dev_info_data, DICS_FLAG_GLOBAL, 0, DIREG_DEV, KEY_READ);
            ...
            default:
                ...
                if (parent_dev == NULL)
                {
                    usbi_dbg("unlisted ancestor for '%s' (non USB HID, newly connected, etc.) - "
                             "ignoring",
                             dev_id_path);
                    continue;
                }
            ...
            { // For subsequent passes, we'll lookup the parent
                ...
                if (dev == NULL)
                {
                    if (pass == DEV_PASS)
                    {
                        // This can occur if the OS only reports a newly plugged device after we
                        // started enum
                        usbi_warn(ctx,
            ...
            case DEV_PASS:
                ...
                default:
                    ...
                    {
                        safe_strcpy(
                            priv->usb_interface[0].path, safe_strlen(priv->path) + 1, priv->path);
                    }
                    else
                    {
            ...
            case GEN_PASS:
                r = init_device(
                    dev, parent_dev, (uint8_t)port_nr, dev_id_path, dev_info_data.DevInst);
                if (r == LIBUSB_SUCCESS)
                {
            ...
            default: // HID_PASS and later
                ...
                else if (parent_priv->apib->id == USB_API_COMPOSITE)
                {
                    ...
                    switch (set_composite_interface(
                        ctx, parent_dev, dev_interface_path, dev_id_path, api, sub_api))
                    {
                    case LIBUSB_SUCCESS:
                        dev_interface_path = NULL;
                        break;
                    case LIBUSB_ERROR_ACCESS:
                        // interface has already been set => make sure dev_interface_path is freed
                        // then
                        break;
                    default:
                        LOOP_BREAK(r);
...
static int
windows_get_device_descriptor(struct libusb_device *dev, unsigned char *buffer, int *host_endian)
{
    struct windows_device_priv *priv = _device_priv(dev);
                                                ...
                                                int                  *host_endian)
{
    ...
    // config index is zero based
    return windows_get_config_descriptor(
        dev, (uint8_t)(priv->active_config - 1), buffer, len, host_endian);
}
...
static int
windows_set_interface_altsetting(struct libusb_device_handle *dev_handle, int iface, int altsetting)
{
    int                         r    = LIBUSB_SUCCESS;
    struct windows_device_priv *priv = _device_priv(dev_handle->dev);
...
static int submit_bulk_transfer(struct usbi_transfer *itransfer)
{
    ...
    usbi_add_pollfd(
        ctx, transfer_priv->pollable_fd.fd, (short)(IS_XFERIN(transfer) ? POLLIN : POLLOUT));
...
static void
windows_transfer_callback(struct usbi_transfer *itransfer, uint32_t io_result, uint32_t io_size)
{
    struct libusb_transfer     *transfer = USBI_TRANSFER_TO_LIBUSB_TRANSFER(itransfer);
    struct windows_device_priv *priv     = _device_priv(transfer->dev_handle->dev);
    int                         status, istatus;
...
static void
windows_handle_callback(struct usbi_transfer *itransfer, uint32_t io_result, uint32_t io_size)
{
    struct libusb_transfer *transfer = USBI_TRANSFER_TO_LIBUSB_TRANSFER(itransfer);
...
static int windows_handle_events(struct libusb_context *ctx,
                                 struct pollfd         *fds,
                                 POLL_NFDS_TYPE         nfds,
                                 int                    num_ready)
{
...
unsigned __stdcall windows_clock_gettime_threaded(void *param)
{
    ...
    while (1)
    {
        ...
        case WM_TIMER_REQUEST:
            ...
            request->tp->tv_nsec =
                (long)(((hires_counter.QuadPart % hires_frequency) / 1000) * hires_ticks_to_ps);
...
static int windows_clock_gettime(int clk_id, struct timespec *tp)
{
    ...
    case USBI_CLOCK_MONOTONIC:
        if (timer_thread)
        {
            ...
            {
                usbi_err(
                    NULL, "PostThreadMessage failed for timer thread: %s", windows_error_str(0));
            ...
            do
            {
                ...
                if (r == WAIT_TIMEOUT)
                {
                    usbi_dbg("could not obtain a timer value within reasonable timeframe - too "
                             "much load?");
                }
 ...
 * USB API backends
 */
static int unsupported_init(int sub_api, struct libusb_context *ctx) { return LIBUSB_SUCCESS; }
static int unsupported_exit(int sub_api) { return LIBUSB_SUCCESS; }
...
static int
unsupported_configure_endpoints(int sub_api, struct libusb_device_handle *dev_handle, int iface)
{
    PRINT_UNSUPPORTED_API(configure_endpoints);
}
static int
unsupported_claim_interface(int sub_api, struct libusb_device_handle *dev_handle, int iface)
{
    PRINT_UNSUPPORTED_API(claim_interface);
}
static int unsupported_set_interface_altsetting(int                          sub_api,
                                                struct libusb_device_handle *dev_handle,
                                                int                          iface,
                                                int                          altsetting)
{
...
static int
unsupported_release_interface(int sub_api, struct libusb_device_handle *dev_handle, int iface)
{
    PRINT_UNSUPPORTED_API(release_interface);
}
static int
unsupported_clear_halt(int sub_api, struct libusb_device_handle *dev_handle, unsigned char endpoint)
{
...
static int
unsupported_copy_transfer_data(int sub_api, struct usbi_transfer *itransfer, uint32_t io_size)
{
...
static int winusbx_init(int sub_api, struct libusb_context *ctx)
{
    ...
    else
    {
        ...
        if (pLibK_GetVersion != NULL)
        {
            ...
            usbi_dbg("libusbK version: %d.%d.%d.%d",
                     LibK_Version.Major,
                     LibK_Version.Minor,
                     LibK_Version.Micro,
                     LibK_Version.Nano);
...
static int winusbx_exit(int sub_api) { return LIBUSB_SUCCESS; }
...
static int winusbx_open(int sub_api, struct libusb_device_handle *dev_handle)
{
    ...
    for (i = 0; i < USB_MAXINTERFACES; i++)
    {
        ...
        {
            file_handle = CreateFileA(priv->usb_interface[i].path,
...
winusbx_configure_endpoints(int sub_api, struct libusb_device_handle *dev_handle, int iface)
{
    ...
    // see
    // http://download.microsoft.com/download/D/1/D/D1DD7745-426B-4CC3-A269-ABBBE427C0EF/DVC-T705_DDC08.pptx
    for (i = -1; i < priv->usb_interface[iface].nb_endpoints; i++)
    {
        endpoint_address = (i == -1) ? 0 : priv->usb_interface[iface].endpoint[i];
        if (!WinUSBX[sub_api].SetPipePolicy(
                winusb_handle, endpoint_address, PIPE_TRANSFER_TIMEOUT, sizeof(ULONG), &timeout))
        {
            usbi_dbg("failed to set PIPE_TRANSFER_TIMEOUT for control endpoint %02X",
                     endpoint_address);
        }
        ...
        policy = false;
        if (!WinUSBX[sub_api].SetPipePolicy(
                winusb_handle, endpoint_address, SHORT_PACKET_TERMINATE, sizeof(UCHAR), &policy))
        {
            usbi_dbg("failed to disable SHORT_PACKET_TERMINATE for endpoint %02X",
                     endpoint_address);
        }
        if (!WinUSBX[sub_api].SetPipePolicy(
                winusb_handle, endpoint_address, IGNORE_SHORT_PACKETS, sizeof(UCHAR), &policy))
        {
        ...
        if (!WinUSBX[sub_api].SetPipePolicy(
                winusb_handle, endpoint_address, ALLOW_PARTIAL_READS, sizeof(UCHAR), &policy))
        {
            usbi_dbg("failed to enable ALLOW_PARTIAL_READS for endpoint %02X", endpoint_address);
        }
        if (!WinUSBX[sub_api].SetPipePolicy(
                winusb_handle, endpoint_address, AUTO_CLEAR_STALL, sizeof(UCHAR), &policy))
        {
        ...
        if (!WinUSBX[sub_api].Initialize(file_handle, &winusb_handle))
        {
            ...
            default:
                ...
                for (i = 0;; i++)
                {
                    ...
                    // ignore GUID part
                    dev_path_no_guid =
                        sanitize_path(strtok(dev_interface_details->DevicePath, "{"));
                    if (safe_strncmp(dev_path_no_guid,
                                     priv->usb_interface[iface].path,
                                     safe_strlen(dev_path_no_guid)) == 0)
                    {
                        file_handle = CreateFileA(filter_path,
                                                  GENERIC_WRITE | GENERIC_READ,
                                                  FILE_SHARE_WRITE | FILE_SHARE_READ,
                                                  NULL,
                                                  OPEN_EXISTING,
                        ...
                        else
                        {
                            ...
                            if (WinUSBX[sub_api].Initialize(file_handle, &winusb_handle))
                                ...
                                usbi_err(
                                    ctx, "could not initialize filter driver for %s", filter_path);
                        }
                    }
                }
                if (!found_filter)
                {
                    usbi_err(
                        ctx, "could not access interface %d: %s", iface, windows_error_str(err));
                    return LIBUSB_ERROR_ACCESS;
                }
            }
        }
    ...
    else
    {
        ...
        if ((winusb_handle == 0) || (winusb_handle == INVALID_HANDLE_VALUE))
        {
            ...
            if (WinUSBX[sub_api].Initialize(file_handle, &winusb_handle))
            {
                ...
                usbi_warn(
                    ctx, "auto-claimed interface 0 (required to claim %d with WinUSB)", iface);
            }
            else
            {
                usbi_warn(ctx,
                          "failed to auto-claim interface 0 (required to claim %d with WinUSB): %s",
                          iface,
                          windows_error_str(0));
                return LIBUSB_ERROR_ACCESS;
            }
...
static int winusbx_submit_control_transfer(int sub_api, struct usbi_transfer *itransfer)
{
    ...
    {
        if (!WinUSBX[sub_api].ControlTransfer(wfd.handle,
                                              *setup,
...
static int winusbx_submit_bulk_transfer(int sub_api, struct usbi_transfer *itransfer)
{
    ...
    if (IS_XFERIN(transfer))
    {
        ...
        ret = WinUSBX[sub_api].ReadPipe(wfd.handle,
                                        transfer->endpoint,
                                        transfer->buffer,
                                        transfer->length,
                                        NULL,
    ...
    else
    {
        ...
        ret = WinUSBX[sub_api].WritePipe(wfd.handle,
                                         transfer->endpoint,
                                         transfer->buffer,
                                         transfer->length,
                                         NULL,
        ...
        if ((winusb_handle != 0) && (winusb_handle != INVALID_HANDLE_VALUE))
        {
            for (j = 0; j < priv->usb_interface[i].nb_endpoints; j++)
            {
                ...
                if (IS_EPIN(priv->usb_interface[i].endpoint[j]) &&
                    (!WinUSBX[sub_api].FlushPipe(winusb_handle,
                                                 priv->usb_interface[i].endpoint[j])))
                {
                    usbi_err(ctx,
...
static int _hid_get_config_descriptor(struct hid_device_priv *dev, void *data, size_t *size)
{
    ...
    config_total_len = LIBUSB_DT_CONFIG_SIZE + LIBUSB_DT_INTERFACE_SIZE + LIBUSB_DT_HID_SIZE +
                       num_endpoints * LIBUSB_DT_ENDPOINT_SIZE;
...
static int
_hid_get_string_descriptor(struct hid_device_priv *dev, int _index, void *data, size_t *size)
{
    void  *tmp      = NULL;
    size_t tmp_size = 0;
    int    i;
...
static int _hid_get_descriptor(struct hid_device_priv *dev,
                               HANDLE                  hid_handle,
                               int                     recipient,
                               int                     type,
                               int                     _index,
...
static int _hid_get_report(struct hid_device_priv       *dev,
                           HANDLE                        hid_handle,
                           int                           id,
                           void                         *data,
                           struct windows_transfer_priv *tp,
                           size_t                       *size,
                           ...
                           int                           report_type)
{
                         ...
                         overlapped))
    {
        ...
        tp->hid_buffer = buf;
        tp->hid_dest =
            (uint8_t *)data; // copy dest, as not necessarily the start of the transfer buffer
        return LIBUSB_SUCCESS;
    }
...
static int _hid_set_report(struct hid_device_priv       *dev,
                           HANDLE                        hid_handle,
                           int                           id,
                           void                         *data,
                           struct windows_transfer_priv *tp,
                           ...
                           int                           report_type)
{
    ...
    // NB: The size returned by DeviceIoControl doesn't include report IDs when not in use (0)
    if (!DeviceIoControl(
            hid_handle, ioctl_code, buf, write_size, buf, write_size, &write_size, overlapped))
    {
        if (GetLastError() != ERROR_IO_PENDING)
        {
    ...
    *size = write_size;
    ...
    {
        usbi_dbg(
            "program assertion failed - write completed synchronously, but no data was written");
    }
    safe_free(buf);
    return LIBUSB_COMPLETED;
}

static int _hid_class_request(struct hid_device_priv       *dev,
                              HANDLE                        hid_handle,
                              int                           request_type,
                              int                           request,
                              int                           value,
...
static int hid_exit(int sub_api) { return LIBUSB_SUCCESS; }
...
static int hid_open(int sub_api, struct libusb_device_handle *dev_handle)
{
    ...
    for (i = 0; i < USB_MAXINTERFACES; i++)
    {
        ...
        {
            hid_handle = CreateFileA(priv->usb_interface[i].path,
                                     GENERIC_WRITE | GENERIC_READ,
                                     FILE_SHARE_WRITE | FILE_SHARE_READ,
                                     NULL,
                                     OPEN_EXISTING,
                                     ...
                                     NULL);
            /*
             * http://www.lvr.com/hidfaq.htm: Why do I receive "Access denied" when attempting to
             * access my HID? "Windows 2000 and later have exclusive read/write access to HIDs that
             * are configured as a system keyboards or mice. An application can obtain a handle to a
             * system keyboard or mouse by not requesting READ or WRITE access with CreateFile.
             * Applications can then use HidD_SetFeature and HidD_GetFeature (if the device supports
             * Feature reports)."
             */
    ...
    do
    {
        ...
        for (j = HidP_Input; j <= HidP_Feature; j++)
        {
            ...
            if (size[j] > 0)
            {
                ...
                if ((value_caps != NULL) &&
                    (HidP_GetValueCaps((HIDP_REPORT_TYPE)j, value_caps, &size[j], preparsed_data) ==
                     HIDP_STATUS_SUCCESS) &&
                    (size[j] >= 1))
                {
                    nb_ids[0] = 0;
        ...
        if (priv->hid->string_index[0] != 0)
        {
            HidD_GetManufacturerString(
                hid_handle, priv->hid->string[0], sizeof(priv->hid->string[0]));
        }
        else
        {
        ...
        if (priv->hid->string_index[2] != 0)
        {
            HidD_GetSerialNumberString(
                hid_handle, priv->hid->string[2], sizeof(priv->hid->string[2]));
        }
        else
        {
...
static int hid_submit_control_transfer(int sub_api, struct usbi_transfer *itransfer)
{
    ...
    case LIBUSB_REQUEST_TYPE_STANDARD:
        ...
        case LIBUSB_REQUEST_GET_DESCRIPTOR:
            r = _hid_get_descriptor(priv->hid,
                                    wfd.handle,
                                    LIBUSB_REQ_RECIPIENT(setup->request_type),
                                    (setup->value >> 8) & 0xFF,
                                    setup->value & 0xFF,
                                    transfer->buffer + LIBUSB_CONTROL_SETUP_SIZE,
                                    &size);
    ...
    {
        // Force request to be completed synchronously. Transferred size has been set by previous
        // call
        wfd.overlapped->Internal = STATUS_COMPLETED_SYNCHRONOUSLY;
 ...
 */
static int composite_init(int sub_api, struct libusb_context *ctx) { return LIBUSB_SUCCESS; }
...
{
    struct windows_device_priv *priv = _device_priv(dev_handle->dev);
    return priv->usb_interface[iface].apib->claim_interface(
                                              ...
                                              int                          altsetting)
{
    ...
    return priv->usb_interface[iface].apib->set_interface_altsetting(
        priv->usb_interface[iface].sub_api, dev_handle, iface, altsetting);
}
...
{
    struct windows_device_priv *priv = _device_priv(dev_handle->dev);
    return priv->usb_interface[iface].apib->release_interface(
        priv->usb_interface[iface].sub_api, dev_handle, iface);
}
...
static int composite_submit_control_transfer(int sub_api, struct usbi_transfer *itransfer)
{
    ...
    case LIBUSB_RECIPIENT_ENDPOINT:
        r = libusb_get_config_descriptor(
            transfer->dev_handle->dev, (uint8_t)(priv->active_config - 1), &conf_desc);
        if (r == LIBUSB_SUCCESS)
        {
    ...
    if ((iface >= 0) && (iface < USB_MAXINTERFACES))
    {
        ...
        if (priv->usb_interface[iface].path != NULL)
        {
            r = priv->usb_interface[iface].apib->submit_control_transfer(
                priv->usb_interface[iface].sub_api, itransfer);
            if (r == LIBUSB_SUCCESS)
            {
                return r;
            }
    ...
    for (pass = 0; pass < 2; pass++)
    {
        for (iface = 0; iface < USB_MAXINTERFACES; iface++)
        {
            if (priv->usb_interface[iface].path != NULL)
            {
                ...
                r = priv->usb_interface[iface].apib->submit_control_transfer(
                    priv->usb_interface[iface].sub_api, itransfer);
                // If not supported on this API, it may be supported on another, so don't give up
                // yet!!
...
static int composite_submit_bulk_transfer(int sub_api, struct usbi_transfer *itransfer)
{
    ...
    return priv->usb_interface[current_interface].apib->submit_bulk_transfer(
        priv->usb_interface[current_interface].sub_api, itransfer);
}
...
composite_clear_halt(int sub_api, struct libusb_device_handle *dev_handle, unsigned char endpoint)
{
    ...
    return priv->usb_interface[current_interface].apib->clear_halt(
        priv->usb_interface[current_interface].sub_api, dev_handle, endpoint);
}
...
static int composite_abort_control(int sub_api, struct usbi_transfer *itransfer)
{
    ...
    return priv->usb_interface[transfer_priv->interface_number].apib->abort_control(
        priv->usb_interface[transfer_priv->interface_number].sub_api, itransfer);
}
...
static int composite_abort_transfers(int sub_api, struct usbi_transfer *itransfer)
{
    ...
    return priv->usb_interface[transfer_priv->interface_number].apib->abort_transfers(
        priv->usb_interface[transfer_priv->interface_number].sub_api, itransfer);
}
...
composite_copy_transfer_data(int sub_api, struct usbi_transfer *itransfer, uint32_t io_size)
{
    ...
    return priv->usb_interface[transfer_priv->interface_number].apib->copy_transfer_data(
        priv->usb_interface[transfer_priv->interface_number].sub_api, itransfer, io_size);
}
